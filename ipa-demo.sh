...
cat $localserversh | ssh $sshopt -i $cert_filename root@"$serverip" "cat ->>~/$serversh" &>> $logfile
...
ssh $sshopt root@$serverip -i $cert_filename "sudo sh ~/$serversh -d $domain -c $serverhostname -r $realm -p $password -e $password" &>> $logfile
...
while [ $clientcnt -lt $clientnr ]; do
	...
	cat $localclientsh | ssh $sshopt -i $cert_filename root@"$clientip" "cat ->>~/$clientsh" &>> $logfile
